...
**Correction, checked against actual git history (2026-09-03), not assumed:** an earlier version of this brief said this repo mainly receives finished work via mirror from the private `agent-builder` upstream, with real development happening there. That's wrong for current practice. Only 4 commits in this repo's entire history are genuinely labeled as mirror syncs (`23fcdc2` "Initial public mirror," two "Release free (WPorg)" commits, most recently `2235d8a` on 2026-08-03, v3.3.46). Every one of the 65 commits since then — taking the plugin from v3.3.47 to v3.3.89, including the Skills overhaul, the MCP tab, i18n, and the WP.org export tooling described in this repo's own `CLAUDE.md` — was committed **directly into this repo**, not received via mirror. The mirror mechanism exists but has been dormant for a month; this repo has clearly been the active, direct development surface that whole time. **Build this feature directly here.** Don't wait for it to arrive from `agent-builder` via a pipeline that isn't currently running.
