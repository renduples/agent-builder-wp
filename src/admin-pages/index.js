...
// Basic-mode Skills embeds a chat with the bundled Skills Assistant instead
// of the raw table — same "small dedicated component hitting agentic/v1/chat
// directly" pattern assets/js/editor-sidebar.js already proved out for the
// Gutenberg sidebar, adapted to this file's real React/JSX toolchain.

function generateSkillsChatSessionId() {
 ...
 */
function ProposalCard( { proposal, sessionId } ) {
	const [ busy, setBusy ] = useState( false );
...
function SkillsChatEmbed( { assistant } ) {
	const [ messages, setMessages ] = useState( [] );
	const [ input, setInput ] = useState( '' );
	const [ sending, setSending ] = useState( false );
	const sessionIdRef = useRef( null );
	if ( null === sessionIdRef.current ) {
		sessionIdRef.current = generateSkillsChatSessionId();
	}
	...
	if ( ! assistant || ! assistant.active ) {
		return (
			<Notice status="info" isDismissible={ false }>
				{ __(
					'The Skills Assistant is still activating.',
					'agent-builder'
				) }{ ' ' }
	...
	const sendMessage = async ( text ) => {
		...
		try {
			const data = await apiFetch( {
				...
				data: {
					message: trimmed,
					agent_id: 'skills-assistant',
					session_id: sessionIdRef.current,
					history,
					deployment_context: 'admin_page_skills',
				},
			} );
	...
	return (
		<div className="agentic-chat-container agentic-skills-chat-embed">
	...
	switch ( data.page ) {
		...
		case 'skills':
			body = data.is_advanced ? (
				...
				<SkillsChatEmbed assistant={ data.assistant } />
			);
			break;
		case 'approvals':
